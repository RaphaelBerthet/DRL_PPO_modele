...
class Reseau_neurones_principal:
    ...
    def entrainement_reseau(self, samples: NDArray[np.float32], reseau_V_estimate: Reseau_neurones_V_estimate):
        for _ in range(NB_ENTRAINEMENT_BATCH):
            ...
            # Probabilité des l'action effectivement jouée
            prob_new = A3[np.arange(TAILLE_BATCHS), actions_batch]
            ratio = np.divide(prob_new, (prob_old_batch + 1e-8))

            one_hot = np.zeros(
                (TAILLE_BATCHS, self.NB_ACTIONS_POSSIBLE),
                dtype=np.float32)
            one_hot[np.arange(TAILLE_BATCHS), actions_batch] = 1.0

            V_next = reseau_V_estimate.calcul_V_estimate(states2_batch)
            V_curr = reseau_V_estimate.calcul_V_estimate(states_batch)
            advantages = rewards_batch + gamma * (1.0 - terminal_state_batch) * V_next - V_curr
            # normalisation des avantages par batch
